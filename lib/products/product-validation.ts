...
export const productShema = z.object({
  name: z
    ...
    .max(120, { message: "Name must be less than 120 character" }),
  slug: z
    .string()
    .min(3, { message: "Slug must be less than 140" })
    .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/, {
      message:
        "Slug must be lowercase and contain only letters, number and hyphens",
    }),
  tagline: z
    .string()
